...
(function() {
  ...
  burger.addEventListener("click", evt => {
    ...
    for (let i = 0; i < menuLinksWrap.length; i++) {
      if (menuLinksWrap[i].classList.contains("menu__links_active")) {
        menuLinksWrap[i].classList.remove("menu__links_active");
  ...
  if (screen.width <= 970) {
    for (let i = 0; i < menuItems.length; i++) {
      menuItems[i].addEventListener('click', () => {
        if (menuLinksWrap[i]) {
          menuLinksWrap[i].classList.toggle("menu__links_active");
